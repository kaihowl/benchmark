        ...
        for i in range(self._olapUser + self._tolapUser, self._olapUser + self._tolapUser + self._oltpUser):
         ...
         if (self._olapUser + self._oltpUser + self._tolapUser) == 0:
            for i in range(self._numUsers):
                self._users.append(self._userClass(userId=i, host=self._host, port=self._port, dirOutput=self._dirResults, queryDict=self._queryDict, collectPerfData=self._collectPerfData, useJson=self._useJson, **s\
elf._userArgs))
